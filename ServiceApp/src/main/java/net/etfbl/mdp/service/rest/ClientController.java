	...
	public Response register(Client client) {
		try {
			if (client == null || client.getUsername() == null || client.getPassword() == null) {
				return Response.status(Response.Status.BAD_REQUEST).entity("{\"message\":\"Invalid client data\"}")
						.build();
			}
			...
			if (ok) {
				...
				return Response.status(Response.Status.CONFLICT).entity("{\"message\":\"User already exists\"}")
						.build();
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Response login(LoginRequest req) {
		try {
			if (req == null || req.username == null || req.password == null) {
				return Response.status(Response.Status.BAD_REQUEST).entity("{\"message\":\"Invalid credentials\"}")
						.build();
			}
			...
			if (!ok) {
				return Response.status(Response.Status.UNAUTHORIZED).entity("{\"message\":\"Wrong password\"}").build();
			}
